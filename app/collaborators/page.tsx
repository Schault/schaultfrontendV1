...
const COLLABORATORS = [
  {
    ...
    whyCards: [
      { icon: "♻️", title: "Reducing Material Waste", desc: "Biodegradable construction means zero landfill contribution. SCHAULT's modular soles mean fewer full-shoe discards." },
      { icon: "🌍", title: "Carbon Footprint", desc: "GRS-certified recycled materials and organic sourcing keep carbon output minimal — aligned with SCHAULT's sustainable manufacturing goals." },
      { icon: "🌿", title: "Soil Pollution", desc: "Traditional synthetic shoes leach microplastics into the soil for decades. Kapas Paduka's natural decomposition ethos directly counters this." },
      { icon: "🌱", title: "Sustainability Vision", desc: "8 trees planted per sale. Bamboo packaging. Bio-based foam. Holistic thinking that complements SCHAULT's design-for-longevity philosophy." },
    ],
  ...
  },
  {
    ...
    whyCards: [
      { icon: "🏭", title: "Established Since 2011", desc: "Over a decade of continuous operation building trust with European buyers — institutional knowledge that de-risks SCHAULT's manufacturing at scale." },
      { icon: "✅", title: "Certified Materials", desc: "PCP/AZO-free leather certification is non-negotiable for European markets. Expose Trendze already operates to these standards." },
      { icon: "🌐", title: "Global Export Network", desc: "Active connections in Germany, Portugal and Italy give SCHAULT a direct path into premium European footwear retail." },
      { icon: "🔬", title: "Sample-First Process", desc: "Free sampling before full production — a model that perfectly mirrors SCHAULT's own prototype-first engineering philosophy." },
    ],
  ...
  },
  {
    ...
    whyCards: [
      { icon: "🏪", title: "Pan-India Retail Reach", desc: "Thousands of retail touchpoints and online presence ensure SCHAULT's modular system reaches consumers everywhere." },
      { icon: "🏗️", title: "Manufacturing at Scale", desc: "Decades of industrial manufacturing expertise to help SCHAULT transition from artisanal prototyping to mass production." },
      { icon: "💼", title: "Brand Trust", desc: "Over 70 years of consumer trust makes Liberty a credible launchpad for introducing new footwear technology to mainstream India." },
      { icon: "📦", title: "Supply Chain Mastery", desc: "Vertically integrated operations from raw material sourcing to final delivery — infrastructure that SCHAULT can leverage from day one." },
    ],
  ...
  return (
    <>
      <main ref={container} className="bg-white min-h-screen">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 min-h-[420px]">
            ...
            <div className="collab-hero-text px-4 sm:px-6 md:px-12 lg:px-16 py-16 md:py-20 flex flex-col justify-center">
              ...
              <div className="flex items-center gap-3 mb-5">
                ...
                <span className="font-inter text-xs font-semibold uppercase tracking-[0.22em] text-[#CC0000]">Our Partners</span>
              </div>
              ...
              <h1 className="font-bebas text-5xl sm:text-6xl md:text-[72px] lg:text-[80px] text-black/90 leading-[1.02] tracking-wide mb-7 uppercase">
                Built Together.<br />
                <span className="italic text-[#CC0000]">Better</span><br />
                Together.
              </h1>

              {/* Sub */}
              <p className="font-inter text-[15px] text-black/55 max-w-[440px] leading-[1.8]">
                SCHAULT doesn&apos;t build the future of footwear alone. We align with partners who share our conviction &mdash; that great shoes should be responsibly made, thoughtfully designed, and built to last.
              </p>
            </div>
            ...
            <div className="bg-black relative overflow-hidden flex flex-col items-start justify-end px-8 sm:px-12 py-10 md:py-14 gap-2.5">
              ...
              {[
                ...
                <div key={pill.label} className="hero-pill-item inline-flex items-center gap-2.5 bg-white/[0.07] border border-white/[0.12] px-5 py-2.5 text-white font-inter text-[12px] font-semibold uppercase tracking-[0.14em]">
                  <span className={`w-2 h-2 rounded-full flex-shrink-0 ${pill.green ? "bg-green-500" : "bg-[#CC0000]"}`} />
                  {pill.label}
                </div>
              ))}

              <p className="font-bebas text-[28px] text-white mt-5 leading-snug tracking-wide">
                3 Strategic<br /><span className="text-[#CC0000]">Collaborators.</span>
              </p>
            </div>
          </div>
        </div>
        ...
        {/* ─── PHILOSOPHY BAR ─── */}
        <div className="philosophy-bar bg-[#CC0000] px-4 sm:px-6 md:px-12 py-5 flex items-center justify-between gap-4 overflow-x-auto">
          {["Sustainability", "Biodegradable Materials", "Carbon Footprint Reduction", "Zero Soil Pollution", "World-Class Craftsmanship", "Global Export Standards", "Certified Leather", "Ethical Production"].map((item, i) => (
            <React.Fragment key={item}>
              {i > 0 && <span className="philosophy-item w-[5px] h-[5px] rounded-full bg-white/50 flex-shrink-0" />}
              <p className="philosophy-item font-inter text-[13px] font-bold uppercase tracking-[0.15em] text-white whitespace-nowrap">{item}</p>
            </React.Fragment>
          ))}
        </div>
        ...
        {COLLABORATORS.map((collab, idx) => (
          <section key={collab.num} className="collab-partner border-b border-black/10">
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2`}>
              ...
              {/* Image side */}
              <div className={`relative overflow-hidden min-h-[400px] md:min-h-[480px] group ${idx % 2 === 1 ? "md:order-1" : "md:order-2"}`}>
                <Image
                  src={collab.image}
                  alt={`${collab.name} collaboration partner`}
                  fill
                  className="object-cover grayscale-[10%] transition-transform duration-700 group-hover:scale-[1.04]"
                />
                <div className="absolute bottom-6 left-6 bg-white py-3 px-5 border-l-[3px] border-[#CC0000]">
              ...
              <div className={`collab-animate px-6 sm:px-10 md:px-14 lg:px-16 py-14 md:py-20 flex flex-col justify-center ${idx % 2 === 1 ? "md:order-2" : "md:order-1"}`}>
                <p className="font-inter text-[11px] font-semibold uppercase tracking-[0.2em] text-[#CC0000] mb-3">
                  ...
                  {collab.name.split(" ").map((word, wi) => (
                    <React.Fragment key={wi}>
                      ...
                      {wi === collab.name.split(" ").length - 1 ? (
                        <span className="italic text-[#CC0000]">{word}</span>
                      ) : (
                        word
                      )}
                    </React.Fragment>
                ...
                <div className="collab-animate flex flex-wrap gap-3 mt-3">
                  <a
                    ...
                    className="inline-flex items-center gap-2 bg-black text-white font-inter text-[12px] font-bold uppercase tracking-[0.12em] px-5 py-3 hover:bg-[#CC0000] hover:-translate-y-0.5 transition-all duration-200"
                  >
            ...
            <div className="bg-[#F5F5F5] px-4 sm:px-6 md:px-12 lg:px-16 py-14 md:py-16">
              <div className="max-w-[1440px] mx-auto">
                <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-12">
                  <div>
                    <p className="font-inter text-[11px] font-semibold uppercase tracking-[0.2em] text-[#CC0000] mb-3">Why We Collaborate</p>
                    <h3 className="collab-animate font-bebas text-[32px] md:text-[40px] text-black/90 tracking-wide leading-snug">
                      {collab.whyTitle.split(" ").map((word, wi) => {
                        ...
                        if (emphasize.includes(word)) {
                          return <span key={wi} className="italic text-[#CC0000]">{word} </span>;
                        }
                        return word + " ";
                      })}
                    </h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[2px]">
                  {collab.whyCards.map((card) => (
                    <div key={card.title} className="collab-animate bg-white p-7 border-t-[3px] border-transparent hover:border-[#CC0000] transition-colors duration-200">
        ...
        <section className="joint-vision bg-[#F5F5F5] relative overflow-hidden py-20 md:py-24 px-4 sm:px-6 md:px-12 text-center">
          {/* Top gradient line */}
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[#CC0000] via-[#CC0000] to-black" />

          <p className="font-inter text-[11px] font-semibold uppercase tracking-[0.22em] text-[#CC0000] mb-5">The Bigger Picture</p>
          <h2 className="font-bebas text-[36px] sm:text-[48px] md:text-[64px] text-black/90 leading-[1.08] tracking-wide max-w-[900px] mx-auto mb-8">
            Three Brands.<br />One <span className="italic text-[#CC0000]">Shared Mission.</span>
          </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
            {[
              { icon: "🔩", name: "SCHAULT", desc: "Modular snap-fit architecture. Replace the sole, keep the upper. Zero full-shoe waste." },
              { icon: "🌱", name: "Kapas Paduka", desc: "Biodegradable materials. 8 trees per sale. Nature-safe at end of product life." },
              { icon: "🏆", name: "Expose Trendze", desc: "World-class certified manufacturing. European export standards. Built for global scale." },
              { icon: "👟", name: "Liberty Shoes", desc: "Pan-India retail reach. 70+ years of trust. Scale to bring modular footwear to millions." },
            ].map((item) => (
              <div key={item.name} className="bg-white p-7 text-left border-b-[3px] border-transparent hover:border-[#CC0000] hover:-translate-y-1 transition-all duration-200">
                <span className="text-[26px] mb-3 block">{item.icon}</span>
        ...
        <div className="collab-cta bg-black text-white py-20 md:py-28 px-4 sm:px-6 md:px-12 text-center relative overflow-hidden group">
          {/* Glow */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-[#CC0000] rounded-full blur-[120px] opacity-15 group-hover:opacity-30 transition-opacity duration-700 pointer-events-none" />
          ...
          <div className="relative z-10">
            ...
            <a
              ...
              className="inline-block bg-white text-black font-bebas text-lg px-12 py-5 hover:bg-[#CC0000] hover:text-white transition-colors tracking-widest"
            >
              GET IN TOUCH
            </a>
          </div>
